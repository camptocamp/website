...
class WebsiteFormRecaptcha(models.AbstractModel):
    ...
    @api.model
    def action_validate(self, req_value, ip_addr):
        """Backward compatibility for old implementation.

        Prior to API refactoring pre-validation steps
        were made in the controller, or in any case, was the duty of the caller
        to call `action_validate` w/ a ready request value and an IP address.
        ...
        """
        return self.validate_response(req_value, ip_addr)
